...
**LoRA storage** — `[trainer.model] lora_base_dtype = "bfloat16"` stores frozen
base parameters in BF16 before FSDP materialization. Model-declared FP32
exceptions are preserved. Trainable adapters and `modules_to_save` retain their
construction dtype; `optimization_dtype` and `reduce_dtype` are independent and
must not be lowered to save frozen-weight memory. The storage override requires
LoRA and cannot be combined with the quantization setting. Omit it to preserve
the default storage behavior. Standalone trainers use `[model]`.
